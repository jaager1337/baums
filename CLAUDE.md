...
- **Warm "Dark Coffee" palette refresh (current):** panel + text tones were warmed after auditing four candidate palettes (olive/bone, dark-academia, mine-shaft, and a warm coffee/cream set — the last won; the others drifted cool/green/grey and fought the wood story). **Ground `--bg #13110D` and accent `--walnut #A07850` were deliberately kept** — only panels and text warmed. A first subtle pass (panels ≈ `#362417`) read too close to the ground; panels were lifted for presence, then eased back a notch (the loud `#4d3621/#382616` + 0.42 frame felt too assertive on the Craft panel + Contact card) to the calmer values below.
- **Framed-panel system (showcases & cards):** a panel = a warm-coffee gradient `linear-gradient(var(--panel-top) #44301d → var(--panel-bot) #322110)` + a walnut frame `--frame rgba(160,120,80,0.36)` + an inner walnut double-mat hairline `--mat rgba(160,120,80,0.18)` + a soft ambient shadow. Used by: the Craft map unit (`.origins` frame + `.origins-panel` body) and the Contact card (`.contact-form-box`). **The Collection gallery stage (`.carousel-stage`) is now a BESPOKE clean matted frame, NOT the shared gradient** — see Collection philosophy (an even solid-coffee mat `#3d2a18` + a single walnut fillet; the shared vertical gradient made its top band lighter than the bottom and read as "weird"). Brown reads as a warm accent, NOT a lighter block — keep panels dark+warm, distinguished by frame/shadow, not by lightness. The map viewport (`.origins-map-wrap #100E0A`) stays the darkest layer on purpose.
- Accent is **walnut** `--walnut #A07850` (not "gold") — used for hairlines, CTAs, active states
- Text tones (warmed in the Dark-Coffee refresh): `--stone #F1DABF` "Almond Cream" (headings/primary), `--body #D8C7AC` warm almond (reading paragraphs — kept a notch below headings so long copy doesn't glare), `--platinum #C2BDB5` (labels — near-neutral, left as-is), `--dim #92817A` "Taupe" (muted/micro-labels only — too low-contrast for body copy)
...
- **Desktop:** the carousel sits inside a **clean matted "gallery stage"** — an **even solid-coffee mat** (`.carousel-stage` bg `#3d2a18`, 18px even padding, faint `rgba(160,120,80,0.28)` outer keyline) with a **single crisp walnut fillet** hugging the photo (`.carousel-stage::before`, `inset:18px`, `1px rgba(160,120,80,0.55)`) and a soft ambient shadow, on a **warm walnut depth field + edge vignette** background with **faint stone veining** (`.collection-vein` SVG, ~5% opacity). **This is bespoke — it does NOT use the shared `--panel-top/--panel-bot` gradient** (that gradient runs top→bottom, so as a picture mat it made the top band visibly lighter than the bottom; the user flagged it as "weird" and asked for a "super clean, graphically designed" frame, so it was rebuilt as an even mat + one fillet, redundant inner/outer borders removed). The **name/badge/material sit OVER the image** (`.slide-overlay-top` / `.slide-overlay-bottom` + the top/bottom `.slide-gradient` dark gradients for legibility) — the user reverted the earlier "caption placard beneath" idea back to text-on-image; the `.carousel-plate` placard was removed entirely (HTML/CSS/JS).
